...
SET @p_params= '<NodoXML><LI><FecDesde>20210101</FecDesde><FecHasta>20220101</FecHasta></LI></NodoXML>'
...
AS
	...
	*/
	...
	--***********************
	---Contador: Recuperar el contador instalado a la fecha
	--***********************
	WITH FCTRS AS(
	SELECT DISTINCT 
	  F.facCtrCod
	...
	, C.conDiametro
	, C.conNumSerie
	, ROW_NUMBER() OVER (PARTITION BY F.facCtrCod, F.facFecha ORDER BY  CC.ctcFec DESC) AS RN
	FROM FCTRS AS F
	LEFT JOIN dbo.ctrcon AS CC
	ON CC.ctcCtr = F.facCtrCod
	--AND CC.ctcFec <= F.facFecha
