...
/*
--Tomando como base  [InformesExcel].[Padron_EXCEL]:
--Incluimos las rectificadas e incluimos filtros para sacar en informe de situación
...
EXEC [InformesExcel].[PadronxSituacion_EXCEL] @p_params, @p_error_out OUTPUT, @p_errMsg_out OUTPUT;

--DROP PROCEDURE  [InformesExcel].[PadronxSituacion_EXCEL]
*/

ALTER PROCEDURE [InformesExcel].[PadronxSituacion_EXCEL]
...
AS
	...
	, F.facCod AS [Cod.Factura]
	, F.facVersion AS [Versión]
	, F.facFechaRectif AS [Rectificativa]
	, CASE WHEN F.facNumero IS NULL THEN 'PREFACTURA'
		   WHEN F.facVersion = 1 AND F.facFechaRectif IS NOT NULL THEN 'ORIGINAL|ANULADA'
		   WHEN F.facVersion = 1 THEN 'ORIGINAL'
		   WHEN F.facFechaRectif IS NOT NULL THEN 'CREADA|ANULADA'
		   ELSE 'CREADA' END AS SITUACION	
