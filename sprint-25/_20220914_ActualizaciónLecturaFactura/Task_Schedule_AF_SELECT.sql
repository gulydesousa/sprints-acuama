...
ALTER PROCEDURE [dbo].[Task_Schedule_AF_SELECT] 
...
ON S.tskType = T.tskTType
 WHERE (@tskUser IS NULL OR @tskUser = tskUser)
   ...
   AND (@tasksToExecute IS NULL OR @tasksToExecute = 0 OR (tskStatus = 1 AND tskScheduledDate <= GETDATE() AND T.tskTOverlapping = 0 AND EXISTS(SELECT tskpNumber FROM Task_Parameters WHERE tskUser = tskpUser AND tskType = tskpType AND tskNumber = tskpNumber)))
