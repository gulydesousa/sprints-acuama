...
EXEC Indicadores.Facturas_ConDeuda @fecha, @minDeuda, @diasVencimiento, @usos, @tipoSalida,  @result OUTPUT;

SELECT @result
*/

CREATE PROCEDURE Indicadores.Facturas_ConDeuda(@fecha DATE, @minDeuda MONEY, @diasVencimiento INT,  @usos VARCHAR(MAX), @tipoSalida TINYINT, @result INT OUTPUT)
AS 
