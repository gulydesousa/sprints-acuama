...
DECLARE @facPerCodD AS VARCHAR(6) = '201905';
DECLARE @facPerCodH AS VARCHAR(6) = '202001';
...
ALTER PROCEDURE dbo.Liquidaciones_RIBADESELLA 
...
BEGIN TRY
	...
	ELSE
	BEGIN
		SELECT TOP 1 @IDLOTE = L.liqLoteNum 
		FROM dbo.liquidacionesLotes  AS L 
		WHERE L.liqLoteUsr=@usrCod AND L.liqLoteTipoId=@liqTipoId
		ORDER BY L.liqLoteFecha DESC
	END
	...
	, F.aprNumero
	, I.inmDireccion
	, [CP] = ISNULL(I.inmcpost, C.ctrTitCPos)
	, C.ctrTitCPos
	, I.inmcpost
	, TV.tviaAbreviado
	, POB.pobdes
	...
	, [CODAGUA180] = SUM(IIF(SUBCONCEPTO_COD = 'PA', 1, 0)) 
	, [IMPAGUA180] = SUM(IIF(SUBCONCEPTO_COD = 'PA', [Base_TRIB], 0)) 
	, [IVAAGUA180] = SUM(IIF(SUBCONCEPTO_COD = 'PA', [Impuesto_TRIB], 0)) 

	, [CODBASURA180] = SUM(IIF(SUBCONCEPTO_COD = 'BA', 1, 0)) 
	, [IMPBASURA180] = SUM(IIF(SUBCONCEPTO_COD = 'BA', [Base_TRIB], 0)) 
	, [IVABASURA180] = SUM(IIF(SUBCONCEPTO_COD = 'BA', [Impuesto_TRIB], 0)) 

	, [CODALC180] = SUM(IIF(SUBCONCEPTO_COD = 'AL', 1, 0)) 
	, [IMPALC180] = SUM(IIF(SUBCONCEPTO_COD = 'AL', [Base_TRIB], 0)) 
	, [IVAALC180] = SUM(IIF(SUBCONCEPTO_COD = 'AL', [Impuesto_TRIB], 0)) 

	, [CODCANON180] = SUM(IIF(SUBCONCEPTO_COD = 'CA', 1, 0)) 
	, [IMPCANON180] = SUM(IIF(SUBCONCEPTO_COD = 'CA', [Base_TRIB], 0)) 
	...
	SELECT [EJ]				= FORMAT(facFecha, 'yy')
		 ...
		 , [MODONOT]		='C'
		 , [FECHALIQ]		= FORMAT(fclFecLiq, 'ddMMyy')
		 ...
		 , [FECHA_APREMIO]	= '000000'
		 ------------------------------
		 , [CODAGUA180]		= IIF(T.CODAGUA180>0, 'PA', '')
		 , [IMPAGUA180]		= IIF(T.CODAGUA180>0, FORMAT(CONVERT(INT, ROUND(T.IMPAGUA180, 2)*100), 'D8')			, '')
		 , [IVAAGUA180]		= IIF(T.CODAGUA180>0, FORMAT(CONVERT(INT, ROUND(T.IVAAGUA180, 2)*100), 'D8'), '')

		 , [CODBASURA180]	= IIF(T.CODBASURA180>0, 'BA', '')
		 , [IMPBASURA180]	= IIF(T.CODBASURA180>0, FORMAT(CONVERT(INT, ROUND(T.IMPBASURA180, 2)*100), 'D8'), '')

		 , [CODALC180]		= IIF(T.CODALC180>0, 'AL', '')
		 , [IMPALC180]		= IIF(T.CODALC180>0, FORMAT(CONVERT(INT, ROUND(T.IMPALC180, 2)*100), 'D8'), '')
		 ------------------------------
		 , [CODMIN180] = ''
		 , [IMPMIN180] = ''
		 ...
		 , [CODCANON180] = IIF(T.CODCANON180>0, 'CA', '')
		 , [IMPCANON180] = IIF(T.CODCANON180>0, FORMAT(CONVERT(INT, ROUND(T.IMPCANON180, 2)*100), 'D8'), '')
	...
	--, T.facPerCod
	--, T.facCtrCod
	--, [Importes] = CAST(IMPAGUA180 AS INT) + CAST(IMPBASURA180 AS INT) + CAST(IMPALC180 AS INT) + CAST(IMPCANON180 AS INT) 
	--FROM fLiquidaciones_RIBADESELLA(@RESULT) AS R
	--INNER JOIN #RPT AS T ON T.RN_PK = R.PK
	--LEFT JOIN #TRIBUTOS AS TT
	--ON  T.facCod = TT.tribFacCod
	--AND T.facPerCod = TT.tribPerCod
	--AND T.facCtrCod = TT.tribCtrCod
	--AND T.facVersion = TT.tribFacVersion
	--WHERE (ROUND(T.Base_FAC, 2) <> CAST(PRINCI AS INT) *0.01) 
	--   OR (ROUND(T.Impuesto_FAC, 2) <> CAST(IVA AS INT) *0.01) 
	--WHERE (CAST(IMPAGUA180 AS INT) + CAST(IMPBASURA180 AS INT) + CAST(IMPALC180 AS INT) + CAST(IMPCANON180 AS INT))*0.01 <> ROUND(T.Base2Dec_FAC, 2) 
	--WHERE CAST(IVAAGUA180 AS INT)*0.01 = ROUND(T.Impuesto_FAC, 2)
	--ORDER BY R.PK;
	--@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
