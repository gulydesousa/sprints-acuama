...
--DECLARE @cblPpag SMALLINT=10004
--DECLARE @cblNum INT=5020
--DECLARE @cblLin SMALLINT=1
/*
DECLARE @cblScd SMALLINT=1
DECLARE @cblPpag SMALLINT=10006
DECLARE @cblNum INT=344
DECLARE @cblLin SMALLINT=2

EXEC  dbo.CobLinDes_GenerarDesglosePrioritario @cblScd, @cblPpag, @cblNum, @cblLin, 1
*/


CREATE PROCEDURE dbo.CobLinDes_GenerarDesglosePrioritario
  @cblScd SMALLINT
, @cblPpag SMALLINT
, @cblNum INT
, @cblLin SMALLINT
...
AS
	...
	SELECT  CC.facCod, CC.facCtrCod, CC.facPerCod, CC.facVersion
		...
		, CLD.cldImporte 
		--DR=1: Para quedarnos con uno de los cobros, si hubiera mas de uno.
		, DR = DENSE_RANK() OVER (ORDER BY C.cobFec DESC, C.cobFecReg DESC, C.cobNum DESC, C.cobScd, C.cobPpag)
	FROM dbo.cobros AS C
	INNER JOIN dbo.coblin AS CL
	ON C.cobScd = CL.cblScd
	AND C.cobPpag = CL.cblPpag
	...
	, RN = ROW_NUMBER() OVER (ORDER BY IIF(fclFecLiq IS NULL, 0, 1000) 
										 , IIF([cldDeuda]=0 AND ISNULL(@revertidoPorLiquidacion, 0) <>2, 100, 0)
										 , svcOrgCod
	...
	SET D.DR_ORG = C.DR_ORG
	  , D.RN = C.RN
	  , D.CN_PDTES = IIF(@revertidoPorLiquidacion IS NULL, C.CN_PDTES, CN_NOLIQU)
	...
	SELECT @CLD_LINEAS = COUNT(*) FROM @DEUDA;
	...
	BEGIN	
		...
		UPDATE D SET
		 ...
		 ,[@cldImporte] =	CASE WHEN @cblImporte = 0 THEN 0
							...
							WHEN @revertidoPorLiquidacion=2  THEN CLD.cldImporte*-1
