...
DECLARE @facPerCodD AS VARCHAR(6) = NULL;
DECLARE @facPerCodH AS VARCHAR(6) = NULL;
...
BEGIN TRY
	...
	SELECT [EJ]				= FORMAT(facFecha, 'yy')
		 ...
		 ------------------------------
		 , [CODAGUA180]		= 'PA' --IIF(T.CODAGUA180>0, 'PA', '')
		 ...
		 , [CODCANON180] = 'CA'	--IIF(T.CODCANON180>0, 'CA', '')
		 , [IMPCANON180] = FORMAT(CONVERT(INT, ROUND(T.IMPCANON180, 2)*100), 'D8')	--IIF(T.CODCANON180>0, FORMAT(CONVERT(INT, ROUND(T.IMPCANON180, 2)*100), 'D8'), '')
