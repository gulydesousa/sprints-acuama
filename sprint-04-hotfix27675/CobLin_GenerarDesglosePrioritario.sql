...
/*
DECLARE  @insertarDesgloseGenerado BIT = 1 
	,@cblScd SMALLINT = 1
	,@cblPpag SMALLINT = 102
	,@cblNum INT = 13898
	,@cblLin SMALLINT = 1
...
AS
    ...
    WHERE /*líneas de cobro de esta factura ->*/cobCtr = @contrato AND cblPer = @periodo AND cblFacVersion = @facturaVersion AND cblFacCod = @facturaCodigo AND
          /*sólo cobros anteriores a este ->*/ cobFecReg <= @cobFecReg AND (cblScd <> ISNULL(@cblScd, -1) OR cblPpag <> ISNULL(@cblPpag, -1) OR cblNum <> ISNULL(@cblNum, -1) OR cblLin <> ISNULL(@cblLin, -1))
	...
	*/	
	...
	ELSE IF (@cobrosAnteriores + @cblImporte = @facTotal2) AND  -- Lo comparo redondeado a dos decimales, como las líneas de cobro
			(@facVersionOriginal > 1 AND
			 EXISTS(SELECT fclNumLinea 
						   FROM faclin 
						   WHERE fclFacCod = @facturaCodigo AND 
	...
	*/
	...
	--Primero seleccionamos los servicios de Acuama, es decir, cuyo organismo es NULL. Los últimos serán los servicios de terceros.
	SELECT ROW_NUMBER() OVER(ORDER BY (CASE WHEN svcOrgCod IS NULL THEN svccod ELSE 9999 END) ASC, 
		   svccod) AS numRegistro, fclNumLinea, fclTotal, fclTrfSvCod, fclTrfCod, svcOrgCod 
	FROM faclin fl
	...
	ORDER BY (CASE WHEN svcOrgCod IS NULL THEN svccod ELSE 9999 END) ASC, svccod
	...
	WHILE @@FETCH_STATUS = 0 BEGIN
		...
		END ELSE BEGIN
			...
			*/
			...
			IF ABS(@cblImporte) = @facTotal2 -- Lo comparo redondeado a dos decimales, como las líneas de cobro
			  BEGIN 
			...
			--Si no, lo repartimos asignando el céntimo que sobra a la última línea, teniendo en cuenta el orden establecido
			  BEGIN
				IF @cobrosAnteriores + @cblImporte = @facTotal2 -- Lo comparo redondeado a dos decimales, como las líneas de cobro
				...
				ELSE 
				  BEGIN
					...
					SET @totalQueda =  ABS(@cblImporte) - ABS(@sumCldImporte)
					
					SET @fclImportePte = @fclTotal-@cobradoServicio;

					--SELECT [@cobrosAnteriores]=@cobrosAnteriores
					...
					ELSE IF((@fclImportePte >= @totalQueda) AND @svcOrgCod IS NULL) 
						SET @cldImporte = @totalQueda * SIGN(@cblImporte)
					
					ELSE IF((@fclImportePte < @totalQueda) AND @svcOrgCod IS NULL)
						SET @cldImporte = @fclImportePte * SIGN(@cblImporte)
											
			    	ELSE IF (@svcOrgCod IS NOT NULL AND @sumCldImporte>0)
						SET @cldImporte= (ROUND((CAST(@fclTotal AS DECIMAL(12,4)) / CAST(@facTotal AS DECIMAL(12,4))) * CAST(@totalQueda AS DECIMAL(12,2)),2)) * SIGN(@cblImporte)
					
					ELSE
						SET @cldImporte = 0										
					
					SET @sumCldImporte =  @sumCldImporte + ABS(@cldImporte)					

					--Si es la última línea de factura, y sobra algún céntimo, vamos a meterlo aquí
					IF @numRegistro = @facNumLineas AND @sumCldImporte <> ABS(@cblImporte) BEGIN
						SET @cldImporte = @cldImporte + ((ABS(@cblImporte) - @sumCldImporte)* SIGN(@cblImporte))
					END
				  END
			    END
		END
