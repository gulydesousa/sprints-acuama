...
ALTER PROCEDURE [dbo].[Tasks_Facturas_CierrePorContrato]
...
AS
	...
	BEGIN TRY
		...
		BEGIN
			BEGIN TRY
				...
				--[3015]Cambio de contador: Marcar como facturado
				UPDATE CC SET conCamFacturado = 1								
				FROM dbo.contadorCambio AS CC
				INNER JOIN dbo.ordenTrabajo AS OT
				ON  (CC.conCamOtNum = OT.otnum)
				AND (CC.conCamValidoParaFacturar = 1)				
				AND (CC.conCamFacturado IS NULL OR CC.conCamFacturado = 0)
				AND (OT.otCtrCod = @facContrato);
			
				--[3016]Contabilizar el tipo de operación en la tabla facturas
	...
	--@ZONA_ERROR		--Error en zona									(10000)
	--@PERZONA_ERROR	--Error en perzona								(01000)
	--@SERIES_ERROR		--Error actualizando el numerador de la serie	(00100) 
	--@IMPUESTOS_ERROR	--Error en el cambio de impuestos				(00010)
	--@CC_ERROR			--Error cambio de contador						(00001)
	...
	INSERT INTO @tskLog VALUES('INFO', '@CC_ERROR', @CC_ERROR, NULL, NULL);
