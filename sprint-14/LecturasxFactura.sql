...
/*
	...
	SET @p_params= '<NodoXML><LI><periodoD>202101</periodoD><periodoH>202101</periodoH><zonaD></zonaD><zonaH></zonaH></LI></NodoXML>'

	EXEC [InformesExcel].[LecturasxFactura]  @p_params, @p_error_out OUTPUT, @p_errMsg_out OUTPUT;
*/

ALTER PROCEDURE [InformesExcel].[LecturasxFactura]
...
AS
	...
	SELECT periodoD = M.Item.value('periodoD[1]', 'VARCHAR(6)')
		 ...
		 , FecDesde = CASE WHEN M.Item.value('FecDesde[1]', 'DATE') = '19000101' THEN NULL 
	...
	AND ((F.facLecActFec IS NOT NULL AND CC.ctcFec <= F.facLecActFec) OR (CC.ctcFec <= @AHORA))
