...
AS
	...
	BEGIN TRY
		...
		SELECT F.*
			 ...
			 --SORIA NOS HA PEDIDO QUE EL ORDEN SEA POR NUMERO DE FACTURA
			 , orderFac = RIGHT(REPLICATE('0', 20) + IIF(@EXPLOTACION = 6, F.facNumero, ''), 20)
		FROM FACS AS F 
		...
		ORDER BY orderFac ASC, F.facpercod, F.facctrcod
		OPTION (OPTIMIZE FOR UNKNOWN);
