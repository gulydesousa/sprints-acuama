...
AS
	...
	SELECT @cobrosAnteriores = ISNULL(SUM(ROUND(cblImporte, 2)), 0), @numCobrosAnteriores = COUNT(*) FROM cobros 
	...
	*/	
	...
	IF (@cobrosAnteriores + ROUND(@cblImporte, 2) = 0)  AND
	...
	ELSE IF (@cobrosAnteriores + ROUND(@cblImporte, 2) = @facTotal2) AND  -- Lo comparo redondeado a dos decimales, como las líneas de cobro
			...
			--Si no, lo repartimos asignando el céntimo que sobra a la última línea, teniendo en cuenta el orden establecido
			  BEGIN
				...
				BEGIN -- Lo comparo redondeado a dos decimales, como el cobro
					...
					SELECT @cldImporte, @fclTotal,  @cobradoServicio , @fclTotal - @cobradoServicio
				...
				BEGIN
					...
					DECLARE @fclPdteDevolver BIT  = IIF(@fclImportePte < 0, 1, 0);
					...
					ELSE IF(@fclPdteDevolver = 1 AND SIGN(@fclImportePte) <> SIGN(@totalQueda)  AND @svcOrgCod IS NULL) 
						SELECT @cldImporte = @fclImportePte, @totalQueda = @totalQueda - @fclImportePte;
					
					ELSE IF(@fclPdteDevolver = 1 AND @fclImportePte <= @totalQueda AND @svcOrgCod IS NULL) 
						SET @cldImporte = @totalQueda;
					
					ELSE IF(@fclPdteDevolver = 0  AND @fclImportePte >= @totalQueda AND @svcOrgCod IS NULL) 
						SET @cldImporte = @totalQueda;
