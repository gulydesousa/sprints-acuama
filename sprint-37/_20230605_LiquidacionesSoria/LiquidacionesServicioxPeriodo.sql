...
SET @p_params= '<NodoXML><LI><periodoD>202001</periodoD><periodoH>202012</periodoH><Servicio>3</Servicio></LI></NodoXML>'
...
AS
	...
	DECLARE @CTR AS TABLE(ctrCod INT, RN INT, [TotalCargoFijo] MONEY, [Total] MONEY);

	INSERT INTO @CTR
	SELECT F.facCtrCod
		 , F.RN 
		 , [Total Cargo Fijo] = SUM(F.CargoFijo + ISNULL(B.CargoFijo, 0))
		 , [Total] = SUM(F.Importe + ISNULL(B.Importe, 0))
	FROM #FCL AS F
	...
	BEGIN
		...
		IF EXISTS (SELECT 1 FROM #BONIFICACIONES WHERE facPerCod=@periodo)
			SELECT [Periodo]			= @periodo
				 ...
				 , [Total]				= F.Importe+ ISNULL(B.Importe, 0)
			FROM @CTR AS C
			LEFT JOIN #FCL AS F
			ON F.facCtrCod = C.ctrCod
			AND F.RN = C.RN
