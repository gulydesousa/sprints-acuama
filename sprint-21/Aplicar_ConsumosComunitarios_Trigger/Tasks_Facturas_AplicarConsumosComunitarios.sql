...
WHERE P.przcodper=@facPerCod AND P.przcodzon=@ctrZonCod AND P.przcierrereal IS NULL)
BEGIN 
	SET @spMessage = FORMATMESSAGE('[ERROR]La zona %s no está abierta para el periodo %s. No se puede ejecutar la tarea', @ctrZonCod, @facPerCod); 
	EXEC Trabajo.errorLog_Insert @spName= @spName, @spMessage=@spMessage, @spParams=@msgParams;
	RETURN 0;
END
--..........................
...
AND F.facPerCod = @facPerCod
AND F.facZonCod = @ctrZonCod
...
--*************************
--*************************
--Con esto omitimos el paso del facturas_Trigger 
...
BEGIN TRANSACTION
	...
	BEGIN
		...
		BEGIN
			...
			BEGIN 
				...
				--[01]Mantenemos el criterio original de acuama
	...
	IF(@VERSION_CNS_COMUNITARIOS IN ('2.1.1', '3.1.1'))
