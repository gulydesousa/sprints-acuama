...
SET @p_params= '<NodoXML><LI><periodoD>202101</periodoD><periodoH>202106</periodoH><FecDesde></FecDesde><FecHasta></FecHasta></LI></NodoXML>'
...
BEGIN TRY
	...
	WHERE A.aprNumero IS NULL AND X.facCod IS NULL;
	...
	BEGIN  
		...
		WHERE F.facPerCod = @percod
		AND ([Base_Ayto] + [Base_Principado]) >0 
		AND ([Base_Ayto]>= @MIN_DEUDA OR [Base_Principado]>= @MIN_DEUDA)
