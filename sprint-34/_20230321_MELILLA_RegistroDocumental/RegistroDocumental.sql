...
BEGIN TRY
	...
	DECLARE @params TABLE (FecDesde DATE NULL, FecHasta DATE NULL, fInforme DATETIME, Info VARCHAR(250));
	INSERT INTO @params
	SELECT  FecDesde = CASE WHEN M.Item.value('FecDesde[1]', 'DATE') = '19000101' THEN NULL ELSE M.Item.value('FecDesde[1]', 'DATE') END
			...
			, fInforme = GETDATE()
			, info='Se omiten registros sin fichero asociado'
	...
	--SYR-424826: Solo los registros que tienen ficheros
	...
	--SYR-424826: Solo los registros que tienen ficheros
	--[Num.Ficheros]>0
	HAVING SUM(IIF(E.regEntArcFichero IS NULL, 0, 1))>0
