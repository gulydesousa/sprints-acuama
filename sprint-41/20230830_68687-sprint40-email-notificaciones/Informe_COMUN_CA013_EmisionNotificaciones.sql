/*
DECLARE @tipo VARCHAR(20)='Lectura',--Tipo Incidencia (Lectura, Inspeccion)
	@cartaTipo INT = 34,
	...
	, @tieneEmail BIT = NULL
	, @excluirNoEmision BIT = 1


EXEC ReportingServices.Informe_COMUN_CA013_EmisionNotificaciones @tipo=N'lectura',@cartaTipo=1,@periodo=N'202104',@tieneEmail=1

EXEC ReportingServices.Informe_COMUN_CA013_EmisionNotificaciones @tipo=N'lectura',@cartaTipo=34,@periodo=N'202301'
,@contratoD=NULL,@contratoH=NULL,@zonaD=NULL,@zonaH=NULL,@ruta1D=NULL,@ruta1H=NULL,@ruta2D=NULL,@ruta2H=NULL,@ruta3D=NULL
,@ruta3H=NULL,@ruta4D=NULL,@ruta4H=NULL,@ruta5D=NULL,@ruta5H=NULL,@ruta6D=NULL,@ruta6H=NULL
,@orden=N'Ruta',@filtro=NULL,@forzarTipoCarta=0
,@legal=N'Indiferente',@listado=N'1',@tieneEmail=NULL,@excluirNoEmision=1
*/
...
AS
	...
	--*******************************************************************************
	--feature/68687-sprint40-email-notificaciones	
	SELECT @tipo = LOWER(ISNULL(@tipo, ''))					 
		 , @excluirNoEmision = ISNULL(@excluirNoEmision, 1); --Por defecto siempre se excluyen los que tienen No-Emisión
	

	DECLARE @sqlExcluirNoEmision VARCHAR(250) = IIF(@excluirNoEmision=1,' AND (C.ctrNoEmision IS NULL OR C.ctrNoEmision=0) ', '');

	DECLARE @sqlPorIncidencia AS VARCHAR (250) =  ' %s JOIN dbo.incilec AS I ON I.inlcod = F.%s ';	

	SELECT @sqlPorIncidencia = FORMATMESSAGE(@sqlPorIncidencia
											--Si hay tipo incidencia, hacemos INNER JOIN para traer solo las que tienen esa incidencia
											, IIF(@tipo = '', 'LEFT', 'INNER')  
											--Si el tipo de incidencia es NULL traems las incidencia de lectura (facLecInlCod)
											, IIF(@tipo= 'inspeccion', 'facInsInlCod',  'facLecInlCod'))

	--SELECT [@sqlPorIncidencia]= @sqlPorIncidencia, [@tipoIncidencia]=@tipoIncidencia, [@tipo]=@tipo, [@sqlExcluirNoEmision]=@sqlExcluirNoEmision;
	--*******************************************************************************
	...
	SET @sql = ( 'SELECT  
				...
				AS ctrCod,
				facPerCod,
				C.ctrNoEmision, F.facInsInlCod, F.facLecInlCod,
				CASE WHEN '+ ISNULL(CAST(@listado AS VARCHAR),'NULL') +' = 1 THEN
				...
				INNER JOIN dbo.contratos AS C ON C.ctrcod= F.facCtrCod AND C.ctrversion = F.facCtrVersion ' + @sqlExcluirNoEmision
				+
				@sqlTieneEmail
				+
