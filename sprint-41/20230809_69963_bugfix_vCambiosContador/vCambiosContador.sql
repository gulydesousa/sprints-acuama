--SELECT * FROM [dbo].[vCambiosContador] WHERE origen='CCMASIVO'
...
SELECT OT.otCtrCod
	...
	, CC.conCamConID
	, CC.conCamFecha
	--RN = 1: En caso que la consulta retornara mas de un registro 
	, RN = ROW_NUMBER() OVER(PARTITION BY  R.ctrCod, CC.conCamConID, CC.conCamFecha ORDER BY CC.conCamFecReg DESC)
FROM RESULT AS R
INNER JOIN dbo.ordenTrabajo AS OT
ON  OT.otCtrCod = R.ctrCod 
AND R.[RN*]=1
...
AND CC.conCamConID = R.[I.conId] 
AND CC.conCamFecha = R.[I.ctcFec])
...
LEFT JOIN OTCC AS OT
ON  OT.otCtrCod = R.ctrCod 
AND OT.conCamConID = R.[I.conId] 
AND OT.conCamFecha = R.[I.ctcFec]
AND OT.RN=1
WHERE [RN*]=1;
