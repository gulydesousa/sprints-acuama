...
ALTER PROCEDURE [dbo].[Facturas_SelectPorTipoDeuda] 
	@contrato INT = NULL, @cliente INT = NULL, @repLegal VARCHAR(80) = NULL, @tipoDeuda SMALLINT
AS
	SET NOCOUNT ON;
	...
	INNER JOIN clientes ON clidociden = ctrTitDocIden
	WHERE (@contrato IS NULL OR F.facCtrCod = @contrato) 
	  ...
	  AND (@repLegal IS NULL OR ctrRepresent = @repLegal)
	  AND (@cliente IS NULL OR clicod = @cliente)
