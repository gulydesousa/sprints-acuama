...
LEFT JOIN APTAS AS A ON
A.iAptaCtrCod=C.ctrcod
WHERE (
		  ...
		  (@ruta1H IS NULL OR C.ctrRuta1 <= @ruta1H ) AND
		  (@ruta2H IS NULL OR C.ctrRuta2 <= @ruta2H ) AND
		  (@ruta3H IS NULL OR C.ctrRuta3 <= @ruta3H ) AND
		  (@ruta4H IS NULL OR C.ctrRuta4 <= @ruta4H ) AND
		  (@ruta5H IS NULL OR C.ctrRuta5 <= @ruta5H ) AND
		  (@ruta6H IS NULL OR C.ctrRuta6 <= @ruta6H ) AND
