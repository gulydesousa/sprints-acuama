...
----***************************
, @anadirContratosNuevos BIT = NULL			--Añade contratoss nuevos a la apertura
...
AS
	...
	DECLARE @totalSteps AS INT = 0 + 1; --1Paso por cada factura  +1Paso PerzonaLote_AsignarLote
	...
	BEGIN TRY
		...
		FROM(
			SELECT C.ctrCod, C.ctrVersion, C.ctrZonCod, C.ctrTitCod, C.ctrFecIni, C.ctrFecAnu, C.ctrbaja, 0 AS existePrefactura
			FROM contratos AS C
			INNER JOIN @CTRCON AS CC
			ON CC.ctrCod = C.ctrcod

			WHERE @anadirContratosNuevos = 1 
			AND
				  NOT EXISTS (SELECT facCtrCod 
									FROM facturas f 
									WHERE facCtrCod = C.ctrCod AND 
									facPerCod = @periodo 
							  ) --no tiene pre/factura creada
							  
					AND (@contratoCodigo IS NULL OR C.ctrcod = @contratoCodigo)
			UNION
		...
		BEGIN
			BEGIN TRY
				...
				IF @existePrefactura = 0 OR @explotacion ='AVG' 
					EXEC dbo.Contratos_ObtenerUltimaLecturaFacturacion @codigoContrato, NULL, @lecturaAnterior OUT, @fechaLecturaAnterior OUT;
				ELSE 
		...
		--Refrescamos los lotes cuando se han insertado facturas nuevas
		--Antes, las facturas nuevas iban todas al lote 9999
		SET @spLog = 'PerzonaLote_AsignarLote';
		IF @anadirContratosNuevos = 1
		BEGIN
			BEGIN TRY
				EXEC @LOTES_ERROR = [dbo].[PerzonaLote_AsignarLote] @zona, @periodo;
				--**************
				--[5099]PerzonaLote_AsignarLote
