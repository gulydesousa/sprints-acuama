...
WHERE ( 
		(OT.otFecRechazo IS NOT NULL) --Rechazada 
		OR 
		(OT.otfrealizacion IS NOT NULL AND OT.otfcierre IS NULL) --Realizada + No Cerrada
	  )
...
ON  MMC.mdlMcnCod = C.conMcnCod
AND MMC.mdlCod = C.conMdlCod
...
SELECT --conID y ctrCod del Nuevo Contador: para ver si el contador nuevo está instalado en otro contrato.
	 ...
	 , [Instalado_ctcCtr] = R.otCtrCod
	 , [conCamConID] = R.conID
	 --**********************************
	 ,[otCtrCod]
	, [conCamOtSerScd] = R.otserscd
	, [conCamOtSerCod] = R.otsercod
	, [conCamOtNum] = R.otnum
	, [conCamFecha] = NULL
	, [conCamFecReg]= NULL
	, [conCamLecIns] = R._conCamLecIns
	, [conCamLecRet] = R._conCamLecRet
	, [conCamConsumoAFacturar] = NULL
	, [conCamUsr] = NULL
	, [conCamPrecinto] = CAST(R._conCamPrecinto AS BIT)
