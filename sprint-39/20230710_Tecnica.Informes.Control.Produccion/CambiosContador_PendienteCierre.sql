...
SET @p_params= '<NodoXML><LI><FecDesde>20230104</FecDesde><FecHasta>20230706</FecHasta><Origen>ANY,CCMASIVO</Origen></LI></NodoXML>'  
...
ALTER PROCEDURE [InformesExcel].[CambiosContador_PendienteCierre] 
...
AS  
 ...
 BEGIN TRY  
	...
	ON O.value = OT.otTipoOrigen
	WHERE OT.otottcod = @OT_TIPO_CC
	  AND OT.otfcierre IS NULL
	  AND (@ORIGEN IS NULL OR @ORIGEN='' OR O.[value] IS NOT NULL);
	...
	, [Empleado] = E.eplnom
	, [Pendiente] = IIF(OT.otFecRechazo IS NOT NULL OR OT.otfrealizacion IS NOT NULL, 'ACUAMA', 'APP')
	, [Origen OT] = OT.otTipoOrigen
