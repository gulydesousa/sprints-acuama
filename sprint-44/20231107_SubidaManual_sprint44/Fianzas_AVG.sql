...
ALTER PROCEDURE [InformesExcel].[Fianzas_AVG]
...
AS
	...
	*/
	...
	--***********************
	--Si hay calibres pendientes porque no hay cambios de contador en este contrato
	--...los buscamos en dbo.vContratosServicio_CuotaFijaAVG
	--***********************
	UPDATE F
	SET F.conDiametro = V.Calibre 
	FROM #FIANZAS AS F
	INNER JOIN dbo.vContratosServicio_CuotaFijaAVG AS V
	ON F.facCtrCod = V.ctrCod AND F.conDiametro IS NULL;
